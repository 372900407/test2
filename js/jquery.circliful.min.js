		...
		return this.each(function() {
			...
			if(a(this).data("text") != undefined) {
				...
				if(a(this).data("type") != undefined) {
					...
					a(this).append('<span class="circle-text1">年化收益率</span>');
					a(this).append('<span class="circle-text">' + D + s + "</span>");
					a(this).find(".circle-text").css({
						"top": 67 + "px",
						"font-size": t + "px",
			...
			function k(x, m) {
				...
				if(m == 100) {
					f.save();
					f.translate(-Math.cos(Math.PI * x * 2 + 0.5 * Math.PI) * j + 23, -Math.sin(Math.PI * x * 2 + 0.5 * Math.PI) * j + j);
					f.fillStyle = '#e85247';
					f.beginPath();
					f.strokeStyle = '#e85247';
					f.lineJoin = "round";  
					f.lineWidth = 8;  
					f.fill();
					f.restore()
					//小圆字体
					f.beginPath();
					f.font = "12px Microsoft Yahei";
					f.textAlign = "center";
					f.fillStyle = '#fff';
					f.save();
					f.translate(-Math.cos(Math.PI * x * 2 + 0.5 * Math.PI) * j + 22, -Math.sin(Math.PI * x * 2 + 0.5 * Math.PI) * j + j);
					f.restore();
				} else {
					//小圆
					f.save();
